...
| [Yggdrasil](https://github.com/NorseArchitecture/Yggdrasil) | `Norse.Hosting.*` — hosting runtimes and deployables: web server, worker, migration service, WASM client, MAUI app, and the BlazingStory catalog host (`Hosting.Stories`, Blazor Interactive Server + MCP) |
...
The devcontainer already redirects its own .NET `bin`/`obj` off the tree it shares with the host (`NORSE_BUILD_ARTIFACTS_DIR` in `devcontainer.json`, read by the root `Directory.Build.props`) — onto container-native disk, both for speed (off the WSL/Windows file bridge that mount crosses) and to avoid two OSes' build output colliding in the same files. Bare-metal Visual Studio builds against that same tree need the equivalent: this script sets `NORSE_BUILD_ARTIFACTS_DIR` as a persistent Windows user environment variable (default `<drive>:\tmp\norse-artifacts`, sibling to the `NorseArchitecture` folder — pass `-ArtifactsDir` to override). Skipping this step doesn't break bare-metal builds directly, but it does mean Windows-built `bin`/`obj` (win-x64 RIDs, Windows paths baked into `obj/project.assets.json`) can land inside the tree the devcontainer also builds from and corrupt the container's next build until you manually clean every `bin`/`obj` folder. Restart Visual Studio after running it so it picks up the new environment variable.

`node_modules` needs nothing from this script: realms with an npm build step install straight into `NORSE_BUILD_ARTIFACTS_DIR/{Realm}/node_modules` from the `.csproj` itself — see Naglfar's `DesignSystem.Tokens.csproj`, which copies `package.json` there, runs `npm ci --prefix`, then resolves against the result via `NORSE_NPM_MODULES_DIR` and a Node loader hook. No symlink or junction is involved on either OS, deliberately: a reparse point written by one is unreadable to the other across the 9p-bridged mount, and npm's reify step deletes one it finds at `node_modules` and writes a real directory back onto the shared tree. If an older run of this script left junctions behind, delete them — nothing recreates them.
