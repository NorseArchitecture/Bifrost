...
**Urðarbrunnr's widened scope (staged 2026-07-22):** the rename from `Norse.EntityFramework.*` to `Norse.Persistence.EntityFramework.*` is staged on Urðarbrunnr's own `feature/persistence-namespace-widening` branch — build and tests pass there, but it hasn't merged to `master` or published a new NuGet version yet. Until it does, consuming realms (Midgard, Yggdrasil, Himinbjörg) still reference the old `Norse.EntityFramework.*` package names; don't assume the new namespace is live until Urðarbrunnr's own ship gate clears.
...
**Resolved since last pass:** the `IDeferredSignIn` realm-placement question that used to sit here as item 2 is settled. Asgard's `Abstractions.Web.Server` now declares the contract (PR #28, merged); Midgard's `Infrastructure.Web.Server` implements it against Asgard instead of declaring its own (commit `590f208`, shipped via PR #29 — bundled alongside an unrelated interceptor-visibility fix, not a dedicated PR). Only the ship step remains: Himinbjörg's `feature/identity-web-server` branch (`NorseSignInManager` in `Identity.Web.Server`) needs to rebase onto the new contract and clear its own PR/CI/tag/NuGet gate before it can merge.
