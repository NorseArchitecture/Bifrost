...
2. **`IDeferredSignIn` may be in the wrong realm — Himinbjörg NorseRef'ing Midgard ran aground on 2026-07-14, redesign pending.** Discovered live: Task 2 of the deferred-sign-in fix (`NorseSignInManager` in Himinbjörg's base `Identity` project, `../Glitnir/docs/Heimdall/plans/2026-07-14-deferred-signin-fix.md`) built and tested clean locally under `-p:UseProjectReferences=true`, which resolves the `NorseRef Infrastructure.Web.Server` (Midgard) against local source — but real CI (no project-reference flag, resolves the same NorseRef as a genuine `PackageReference`) failed with `CS0234`/`CS0246`: `Norse.Infrastructure.Web.Server.DeferredSignIn`/`IDeferredSignIn` don't exist, because Midgard's Task 1 (`IDeferredSignIn`/`MemoryCacheDeferredSignIn`/`DeferredSignInEndpointRouteBuilderExtensions`) was still staged on `fix/outcome-server-interceptor-public`, never actually shipped through its own ship gate (PR/CI/tag/NuGet) before Himinbjörg took a dependency on it. Local project-reference success papered over a real unpublished-dependency failure — a sequencing gap, not a code bug.

   Surfacing the sequencing gap raised a sharper question: should Himinbjörg's foundational `Identity` project (its own csproj: "referenced by Identity.Web.Server; never by migration tooling") be NorseRef'ing Midgard's `Infrastructure.Web.Server` (`Norse.Infrastructure.*` — embodied law, concrete persistence/mediator runtime) at all for a Blazor-Server-hosting concern like deferred sign-in? Candidates to actually own `IDeferredSignIn` instead: Asgard (`Norse.Abstractions.*` — declared law, where the *contract* might belong even if an implementation lives elsewhere) or Yggdrasil (`Norse.Hosting.*` — this is fundamentally a hosting-runtime problem, and Yggdrasil already owns "web server, worker, migration service, WASM client, MAUI app"). Open risk not yet resolved: a naive move to Yggdrasil may invert the graph the wrong way — Yggdrasil is the outermost composition/hosting layer, and Himinbjörg's `Identity` depending on it would be backwards (chicken-and-egg, not yet fully traced through the stack). Needs a full-stack pass before redesigning, not a quick fix. Current state: `Himinbjorg/feature/identity-web-server` has correct, fully-tested Task 2 code, not mergeable until this resolves one way or another.
